...
export class MiniChartComponent {
  @Input({ required: true }) set snapshot(ss: SnapshotInfo | undefined) {
    if (ss) {
      this.marketRestSvc
  ...
  private setupChart(data: CandlestickData[], ss: SnapshotInfo) {
    this.chart?.remove();
    ...
    const positive = ss.todaysChange && ss.todaysChange > 0;
